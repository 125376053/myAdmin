<template>
  <div class="register2-wrapper">
    <div class="contain">
      ...
      <p class="tips">您已选择注册开通
        <strong v-show="isOrgan">企业账户</strong>
        <strong v-show="!isOrgan">个人账户</strong>，请完善注册信息，即刻开始使用证书智能管理系统吧！
      </p>
      <el-form ref="personalForm" :rules="rules" :model="personalForm" label-width="1px">
        <el-form-item>
          <el-row :gutter="20">
            <el-col :span="12">
        ...
        <el-form-item>
          <el-button
            ...
            style="width:260px;"
            size="medium"
            @click="savePersonalInfo('personalForm')"
          >保存</el-button>
        </el-form-item>
      </el-form>
...
export default {
  ...
  mounted() {
    ...
    if (Object.keys(this.routerParams).length !== 0) {
      // alert(JSON.stringify(this.routerParams))
      this.$axios
      ...
      // debugger
      // console.log(this.$route.params.userInfo)
      this.getBack = this.$route.params.userInfo;
      this._getCountryList();
    }
  },
  methods: {
    ...
    savePersonalInfo(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.$axios
            .post("/api/register/onCompletion", {
            ...
            .then(res => {
              ...
              if (data.code === 0) {
                localStorage.setItem(
                  "userName",
                  this.personalForm.lastName + this.personalForm.firstName
                );
                this.$router.push({
                  name: "clientRegister3",
                  params: {
                    userInfo: this.getBack,
                    userName:this.personalForm.lastName + this.personalForm.firstName
                  }
                });
              } else {
                this.$message({
